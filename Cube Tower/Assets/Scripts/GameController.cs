...
public class GameController : MonoBehaviour {
    ...
    private void Start() {
        ...
        scoreTxt.text ="<size=23><color='orange'>best:</color></size>"+PlayerPrefs.GetInt("score")+"\n<size=22>now:</size>0";
    ...
    private void SpawnPositions () {
        ...
        if(IsPossitionEmpty(new Vector3(nowCube.x +1, nowCube.y,nowCube.z))&& nowCube.x+1!=cubeToPlace.position.x){
           positions.Add(new Vector3(nowCube.x +1, nowCube.y,nowCube.z));
        } if(IsPossitionEmpty(new Vector3(nowCube.x-1, nowCube.y,nowCube.z))&& nowCube.x-1!=cubeToPlace.position.x) {
           positions.Add(new Vector3(nowCube.x-1, nowCube.y,nowCube.z));
        } if(IsPossitionEmpty(new Vector3(nowCube.x, nowCube.y+1,nowCube.z))&& nowCube.y+1!=cubeToPlace.position.y) {
           positions.Add(new Vector3(nowCube.x, nowCube.y+1,nowCube.z));
        } if(IsPossitionEmpty(new Vector3(nowCube.x, nowCube.y-1,nowCube.z))&& nowCube.y-1!=cubeToPlace.position.y) {
           positions.Add(new Vector3(nowCube.x, nowCube.y-1,nowCube.z));
        } if(IsPossitionEmpty(new Vector3(nowCube.x, nowCube.y,nowCube.z-1))&& nowCube.z+1!=cubeToPlace.position.z) {
    ...
    private bool IsPossitionEmpty(Vector3 targetPos) {
        if(targetPos.y==0)
            return false;
